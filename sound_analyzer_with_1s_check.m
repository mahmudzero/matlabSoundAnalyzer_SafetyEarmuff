...
while number_of_samples < 150
	...
	%this if checks if the fft vector contains a spike at
    %(index - 1)*100hZ, greater than 0.2
    %so for index = 201, we are looking for a 20kHz signal
	if(fspectrum_sound_vector(151) > 0.1)
		threshold_hit = threshold_hit + 1;
        disp(threshold_hit);
        if(threshold_hit > 10)
            threshold_hit = 0;
            options = weboptions('Timeout', 20);
            webwrite("https://safety-earmuff.herokuapp.com/send-mail", "status=found", options);
